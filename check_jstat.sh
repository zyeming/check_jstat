...
if [ $cs -gt 0 -a $heapratio -ge $cs ]; then
    echo "$hostname;$label;2;critical Heap (${heapratio}% of MaxHeapSize)|$perfdata"
    exit 2
fi

if [ $ws -gt 0 -a $permratio -ge $ws ]; then
    echo "$hostname;$label;1;warning PermGen (${permratio}% of MaxPermSize)|$perfdata"
    exit 1
fi
if [ $ws -gt 0 -a $heapratio -ge $ws ]; then
    echo "$hostname;$label;1;warning Heap (${heapratio}% of MaxHeapSize)|$perfdata"
    exit 1
fi
echo "$hostname;$label;0;OK|$perfdata"
exit 0
